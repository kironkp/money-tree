...
class AgentConfigForm(forms.ModelForm):
    class Meta:
        model = AgentConfig
        fields = [
            'trading_enabled', 'news_enabled', 'timeframe', 'crypto_timeframe', 'degen_timeframe', 'pulse_seconds', 'starting_cash', 'risk_per_trade_pct', 'max_position_pct',
            ...
            'degen_max_directional_exposure_pct',
        ...
        widgets = {'timeframe': forms.Select(choices=[(t, t) for t in ('1Min', '5Min', '15Min', '30Min', '1Hour')]),
                   'crypto_timeframe': forms.Select(choices=[(t, t) for t in ('15Min', '30Min', '1Hour', '4Hour')]),
                   'degen_timeframe': forms.Select(choices=[(t, t) for t in ('1Min', '5Min', '15Min', '30Min', '1Hour')]),
